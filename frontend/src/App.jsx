...
const App = () => {
    // State and dispatch from global state provider
    const [{ profile, reloadPage }, dispatch] = useGlobalState();
    const token = window.localStorage.getItem('token');

    // Fetch user profile information on initial load or token change
    useEffect(() => {
        if (token !== null) {
            const getData = async () => {
                Axios({
    ...
    // Fetch user cart information if a profile is available
    useEffect(() => {
        if (profile !== null) {
            const getData = async () => {
                Axios({
